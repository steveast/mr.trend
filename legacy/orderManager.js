...
class OrderManager {
  ...
  static async getCurrentPrice(symbol) {
    try {
      const ticker = await this.signedRequest("GET", "/fapi/v1/ticker/price", {
        symbol,
      });
  ...
  static async placeTakeProfits(
    ...
    for (let i = 1; i <= 10; i++) {
      const tpPrice = this.roundToTick(
        isLong ? stopPrice + step * i : stopPrice - step * i,
        symbol
      );
      const params = {
        symbol,
      ...
      try {
        ...
        console.log(
          `TP${i} ${isLong ? "LONG" : "SHORT"}: ${tpPrice.toFixed(2)}${i === 10 ? " (close)" : ""}`
        );
      } catch (err) {
  ...
  static async getPosition(positionSide) {
    const accountInfo = await OrderManager.signedRequest(
      "GET",
      "/fapi/v2/account"
    );
  ...
  static async moveSLToBreakeven(symbol, positionSide, entryPrice) {
    try {
      ...
      console.log(
        `SL ${positionSide} → безубыток: ${stopPrice.toFixed(2)} (qty: ${qty})`
      );
    } catch (err) {
      console.error(
        `Ошибка безубытка [${symbol} ${positionSide}]:`,
        err.message
      );
    }
  }
